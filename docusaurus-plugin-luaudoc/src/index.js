...
const mapLinks = (nameSet, items) =>
  items.map((name) => {
    if (!nameSet.has(name)) {
      throw new Error(
        `LuauDoc plugin: "${name}" listed in classOrder option does not exist`
      )
    }
...
module.exports = (context, options) => ({
  ...
  async loadContent() {
    ...
    const binaryPath = options.binaryPath ?? "luaudoc-extractor"
      ...
      if (
        ...
        throw new Error(
          "When using autoSectionPath, classOrder cannot contain bare string keys." +
            "Use sectional style instead: https://eryn.io/luaudoc/docs/Configuration#sections"
        )
      }
...
module.exports.validateOptions = ({ options }) => {
  if (!options.code) {
    throw new Error(
      "LuauDoc plugin: expected option `code` to point to your source code."
    )
  }

  if (options.sourceUrl && typeof options.sourceUrl !== "string") {
    throw new Error(
      "LuauDoc plugin: expected option `sourceUrl` to be a string."
    )
  }

  if (options.projectDir && typeof options.projectDir !== "string") {
    throw new Error(
      "LuauDoc plugin: expected option `projectDir` to be a string."
    )
  }
  ...
  for (const [index, codePath] of options.code.entries()) {
    if (typeof codePath !== "string") {
      throw new Error(
        `LuauDoc plugin: code should be an array of strings, found a: ${typeof codePath}`
      )
    }
    ...
    if (!fs.existsSync(resolvedPath)) {
      throw new Error(
        `LuauDoc plugin: code path ${resolvedPath} does not actually exist.`
      )
    }
