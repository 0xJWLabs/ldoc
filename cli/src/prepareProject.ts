...
const SNIP = "<!--luaudoc-hide-before-this-line-->"
...
Either set this project up as a Git repo, or configure the website title in luaudoc.toml`
...
function readConfig(projectDir: string): Config {
  const configPath = path.join(projectDir, "luaudoc")
...
function copyLuauDocFolder(
  projectDir: string,
  tempDir: string
): { customCssExists: boolean; customSidebarExists: boolean } {
  const staticDir = path.join(projectDir, ".luaudoc", "static")
  ...
  const customCssPath = path.join(projectDir, ".luaudoc", "custom.css")
  ...
  const customSidebarsPath = path.join(projectDir, ".luaudoc", "sidebars.js")
...
function needsCompleteRebuild(tempDir: string): boolean {
  ...
  if (
    ...
    console.log(
      "LuauDoc: package.json or package-lock.json does not exist, rebuilding..."
    )
    return true
  }
  ...
  if (
    ...
    console.log(
      "LuauDoc: package.json differs from cached files, rebuilding..."
    )
    return true
  }
...
export function prepareProject(
  ...
  const tempDir = path.join(cachedir("luaudoc"), folderName)
  ...
  if (!options.skipRootCopy) {
    ...
    const luaudocPluginPath = process.env.LUAUDOC_PLUGIN_PATH

    if (process.env.LUAUDOC_DEV || luaudocPluginPath) {
      console.log(
        `LuauDoc: Using development Docusaurus plugin: ${
          process.env.LUAUDOC_PLUGIN_PATH || "../../docusaurus-plugin-luaudoc"
        }`
      )
      const packageJsonPath = path.join(tempDir, "package.json")

      const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, "utf8"))
      packageJson.dependencies["docusaurus-plugin-luaudoc"] =
        luaudocPluginPath
          ? path.resolve(process.cwd(), luaudocPluginPath)
          : path.resolve(__dirname, "../../docusaurus-plugin-luaudoc")
  ...
  const { customCssExists, customSidebarExists } = copyLuauDocFolder(
    projectDir,
    tempDir
  )
  ...
  return {
    ...
    watchPaths: [
      typeof config.home?.includeReadme === "string"
        ? config.home.includeReadme
        : "README.md",
      path.join(projectDir, "luaudoc.toml"),
      path.join(projectDir, "luaudoc.json"),
      path.join(projectDir, "CHANGELOG.md"),
      path.join(projectDir, ".luaudoc/"),
